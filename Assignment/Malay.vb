...
Public Class Malay
    ...
    Private Sub btnClick(sender As Object, e As EventArgs)
        Try
            ...
            For k As Integer = 1 To count
                ...
                picMalay.Visible = True
                malay = sentButton.Text
                i = 0
                sentButton.Focus()
                ...
                While dr.Read
                    ...
                    Using ms As New MemoryStream(data)
                        picMalay.Image = Image.FromStream(ms)
                    End Using
                End While
            Next
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        ...
        Dim sql As String = "select productPrice from [product] p , [user] u 
                             where p.userid = u.userid and productStatus = 'available' and categoryOfProduct = 'malay'
                             and p.productName = ? order by productID"
        Dim cmd As New OleDbCommand(sql, conn)
        cmd.Parameters.AddWithValue("@p1", malay)
        Dim i As Integer = 0
        ...
        Else
            ...
            If Cart.lstCart.Items.Count = 0 Then
                Cart.lstCart.Items.Add(malay & " / " & lblQty.Text)
                exist = True
            Else
                While i < Cart.lstCart.Items.Count
                    If Cart.lstCart.Items.Item(i).ToString.StartsWith(malay) And exist = False Then
                        Cart.lstCart.Items.Add(CStr(malay + " / " & CStr(CDec(Cart.lstCart.Items.Item(i).ToString().Substring(malay.Length + 3)) + CDec(lblQty.Text))))
    ...
    Private Sub malay_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim sql As String = "select productName,productPrice from [product] p , [user] u where p.userid = u.userid and productStatus = 'available' and categoryOfProduct = 'malay'
                             order by productID"
        ...
        While dr.Read()
            ...
            pnlMalay.Controls.Add(btn)
            pnlMalay.Controls.Add(lbl)
            btn.Text = dr(0)
