...
Public Class Western
    ...
    Private Sub btnClick(sender As Object, e As EventArgs)
        Try
            ...
            For k As Integer = 1 To count
                ...
                picWestern.Visible = True
                western = sentButton.Text
                i = 0
                sentButton.Focus()
                ...
                While dr.Read
                    ...
                    Using ms As New MemoryStream(data)
                        picWestern.Image = Image.FromStream(ms)
                    End Using
                End While
            Next
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        ...
        Dim sql As String = "select productPrice from [product] p , [user] u 
                             where p.userid = u.userid and productStatus = 'available' and categoryOfProduct = 'western'
                             and p.productName = ? order by productID"
        Dim cmd As New OleDbCommand(sql, conn)
        cmd.Parameters.AddWithValue("@p1", western)
        Dim i As Integer = 0
        ...
        Else
            ...
            If Cart.lstCart.Items.Count = 0 Then
                Cart.lstCart.Items.Add(western & " / " & lblQty.Text)
                exist = True
            Else
                While i < Cart.lstCart.Items.Count
                    If Cart.lstCart.Items.Item(i).ToString.StartsWith(western) And exist = False Then
                        Cart.lstCart.Items.Add(CStr(western + " / " & CStr(CDec(Cart.lstCart.Items.Item(i).ToString().Substring(western.Length + 3)) + CDec(lblQty.Text))))
    ...
    Private Sub western_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim sql As String = "select productName,productPrice from [product] p , [user] u where p.userid = u.userid and productStatus = 'available' and categoryOfProduct = 'western'
                             order by productID"
        ...
        While dr.Read()
            ...
            pnlWestern.Controls.Add(btn)
            pnlWestern.Controls.Add(lbl)
            btn.Text = dr(0)
