...
Public Class FrmEditDeleteVendor1
        ...
        Try
            While dr.Read()
                If dr(0) = txtUserID.Text Then
                    txtPassword.Text = FrmVendorLogin.txtPassword.Text
    ...
    Private Sub FrmEditDeleteVendor1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        txtUserID.Text = FrmVendorLogin.txtVendorId.Text
        cmbbxCategory.SelectedItem = cmbbxCategory.Items(0)
    End Sub
        ...
        If txtUserID.ReadOnly = True Then
            ...
            If (password.Length >= 8 And System.Text.RegularExpressions.Regex.IsMatch(password, "^[A-Za-z0-9]+$") And
                ...
                cmd = New OleDbCommand("UPDATE [User] SET [UserName] = ?, [IcNo] = ?,
                                ...
                                [CategoryOfProduct] = ?, [securityPattern] = ?, [Password] = ?
                                WHERE [UserID] = ?", con)
                ...
                cmd.Parameters.AddWithValue("@p9", Hash.ComputeHash(password, Nothing))
                cmd.Parameters.AddWithValue("@p10", txtUserID.Text)
                cmd.ExecuteNonQuery()
            ...
            ElseIf count <> 5 Then
                MessageBox.Show("Please select 5 boxes for security purpose")
            End If
        Else
