...
Public Class Asian
    ...
    Private Sub btnClick(sender As Object, e As EventArgs)
        Try
            ...
            For k As Integer = 1 To count
                ...
                picAsian.Visible = True
                asian = sentButton.Text
                i = 0
                sentButton.Focus()
                ...
                While dr.Read
                    ...
                    Using ms As New MemoryStream(data)
                        picAsian.Image = Image.FromStream(ms)
                    End Using
                End While
            Next
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        ...
        Dim sql As String = "select productPrice from [product] p , [user] u 
                             where p.userid = u.userid and productStatus = 'available' and categoryOfProduct = 'asian'
                             and p.productName = ? order by productID"
        Dim cmd As New OleDbCommand(sql, conn)
        cmd.Parameters.AddWithValue("@p1", asian)
        Dim i As Integer = 0
        ...
        Else
            ...
            If Cart.lstCart.Items.Count = 0 Then
                Cart.lstCart.Items.Add(asian & " / " & lblQty.Text)
                exist = True
            Else
                While i < Cart.lstCart.Items.Count
                    If Cart.lstCart.Items.Item(i).ToString.StartsWith(asian) And exist = False Then
                        Cart.lstCart.Items.Add(CStr(asian + " / " & CStr(CDec(Cart.lstCart.Items.Item(i).ToString().Substring(asian.Length + 3)) + CDec(lblQty.Text))))
    ...
    Private Sub Asian_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim sql As String = "select productName,productPrice from [product] p , [user] u where p.userid = u.userid and productStatus = 'available' and categoryOfProduct = 'asian'
                             order by productID"
        ...
        While dr.Read()
            ...
            pnlAsian.Controls.Add(btn)
            pnlAsian.Controls.Add(lbl)
            btn.Text = dr(0)
